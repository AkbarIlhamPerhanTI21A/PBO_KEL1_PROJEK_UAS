...
public class Ujian implements Crud {
    ...
    public boolean create() throws SQLException {
        PreparedStatement ps = Koneksi.getConnection().prepareStatement("Insert Into ujian set id_matkul = ?, nama_matkul = ?, nim = ?, nid = ?, nilai = ?");
        ps.setInt(1, getIdUjian());
        ps.setString(2, getNamaMatkul());
        ps.setString(3, getNim());
        ps.setString(4, getNid());
        ps.setInt(5, getNilai());
        boolean r = false;
        try {
    ...
    public boolean update(Object id) throws SQLException {
        ...
        ps.setString(4, getNid());
        ps.setInt(5, getNilai());
        ps.setInt(6, Integer.parseInt(id.toString()));
        boolean r = false;
        try {
            r = ps.executeUpdate() > 0;
        } catch (Exception e) {
